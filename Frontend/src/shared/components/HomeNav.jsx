...
const HomeNav = () => {
  ...
  // location.pathname == "/" ? setHeader( false );
  useEffect( () => {
  ...
  return (
    <header className="header">
      <div
        className={ `md:px-12 lg:px-24 ${ header ? "bg-white shadow-lg py-3" : "bg-transparent py-8 "
          } fixed z-50 w-full transition-all duration-500` }
      >
        <div className="container flex flex-col items-center mx-auto gap-y-2 md:flex-row md:justify-between md:gap-y-0">
          <a href="/">
            <h4
              className={ `flex gap-2 items-center font-bold text-3xl ${ header ? "text-primary" : "text-white"
                }` }
            >
